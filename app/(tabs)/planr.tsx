...
useEffect(() => {
  ...
  const handleToggle = async (
    ...
    try {
      ...
      await fetch(`http://192.168.45.201:8080/api/meals/user/${mealId}/tick`, {
        method: 'POST',
        headers: {
  ...
  const loadTicks = async (date: string, userId: number) => {
    try {
      ...
      const res = await fetch(
        `http://192.168.45.201:8080/api/meals/user/${userId}/ticks?date=${date}`,
        {
          headers: { 
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}` 
...
const fetchMealPlanForDate = async (date: string, coachId: number) => {
  ...
  console.log(init)
    ...
    try {
      ...
      // 4. Envoyer au back
      const token = await getToken();
      const res = await fetch("http://192.168.45.201:8080/api/progress", {
        method: "POST",
        headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`,
