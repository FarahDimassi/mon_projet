...
import { getUserIdFromToken, getToken, sendNotificationToUser } from "@/utils/authService";
...
import { registerBadgeNotification } from "@/utils/badgeNotificationService";
import { addUserPoints } from "@/utils/badgeService";
  ...
  const checkBadges = async (id: number, currentBonuses: Bonus[]) => {
    try {
      ...
      if (newlyUnlocked.length > 0) {
        ...
        // Enregistrer les badges dans le stockage local
        ...
        for (const badge of newlyUnlocked) {
          // Utiliser le service de notification de badge
          await registerBadgeNotification(id, {
            id: badge.id,
            name: badge.name,
            description: badge.description,
            imageUrl: badge.image,
            points: 50 // Chaque badge vaut 50 points par défaut
          });
          
          // Ajouter les points au localStorage (les points sont déjà ajoutés dans registerBadgeNotification)
          await addUserPoints(id, 50);
        }
      }
