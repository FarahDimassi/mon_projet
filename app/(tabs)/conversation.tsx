...
interface ChatMessage {
  ...
  date?: string;
  isFromMe?: boolean;
  attachmentType?: string; // 'image' ou 'audio'
...
const guessMime = (ext: string) => {
  ...
  // (Optionnel) Rôle de l'utilisateur, ici utilisé uniquement pour choisir le mode d'affichage si nécessaire
  const isCoach = false;

  // Etat pour la recherche (filtrage sur le username, ici partnerName)
  const [searchQuery, setSearchQuery] = useState("");
  
  // État pour déterminer si on est sur le web ou mobile
  ...
  // 1) Récupération du token et de l'ID utilisateur
  // ------------------------------
  useEffect(() => {
    (async () => {
  ...
  // ------------------------------
  // 2) Récupération de la liste des contacts depuis la table "chats"
  // ------------------------------
  useEffect(() => {
  ...
  // Filtrer les contacts en fonction du texte saisi (filtrage sur partnerName)
  const filteredContacts = contacts.filter((contact) =>
    contact.partnerName.toLowerCase().includes(searchQuery.toLowerCase())
  );

  // ------------------------------
  // 3) Lorsque l'on sélectionne un contact, on charge l'historique et on configure le WebSocket
  // ------------------------------
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        // Ajout du flag isFromMe pour différencier l'expéditeur
    ...
    // 3b) Configuration WebSocket / STOMP
  ...
  const sendMessage = () => {
    if (client && message.trim().length > 0 && selectedContact) {
      const chatMessage = {
        ...
        message: message,
        date: new Date(), // Par exemple, initialement un objet Date
      };
      client.publish({
      ...
      // Mise à jour immédiate de l'état chatMessages
      setChatMessages((prev) => [
        ...prev,
        { ...chatMessage, senderId: Number(userId), isFromMe: true, date: new Date().toISOString() },
      ]);
  ...
  // ------------------------------
  // 5) Fonctions de gestion des pièces jointes (image et audio)
  // ------------------------------
  
  // Ouvrir le sélecteur d'options (image ou audio)
  ...
  // Fonction pour prendre une photo adaptée aux plateformes web et mobile
  const takePicture = async () => {
    try {
      if (isWeb) {
        // Pour le web, utiliser l'API web standard (input file)
        ...
        fileInput.onchange = async (e) => {
          ...
          if (file) {
            // Créer un URI pour le fichier
            const fileUri = URL.createObjectURL(file);
            await uploadAndSendImage(fileUri, file);
          }
        };
        
        fileInput.click();
      } else {
        // Pour mobile, utiliser expo-image-picker
  ...
  // Sélectionner une image depuis la galerie (adapté web et mobile)
  ...
  // Uploader et envoyer l'image (adapté web et mobile)
  ...
  // Démarrer l'enregistrement audio (adapté web et mobile)
  const startRecording = async () => {
    try {
      if (isWeb) {
        // Pour le web, utiliser l'API MediaRecorder
        ...
        mediaRecorder.addEventListener("stop", async () => {
          ...
          await uploadAndSendAudio(audioUrl, audioBlob);
        });
        
        mediaRecorder.start();
        
        // Stockage des références pour pouvoir arrêter l'enregistrement plus tard
  ...
  // Arrêter l'enregistrement audio (adapté web et mobile)
  const stopRecording = async () => {
    try {
      ...
      if (isWeb) {
        // Pour le web
        const mediaRecorder = recording as unknown as MediaRecorder;
        if (mediaRecorder && mediaRecorder.state !== 'inactive') {
          mediaRecorder.stop();
          // Le reste du traitement est géré dans l'événement "stop" de mediaRecorder
        }
      } else {
        // Pour mobile
        if (recording) {
  ...
  // Uploader et envoyer l'audio (adapté web et mobile)
  ...
  // Fonction pour lire un audio (adapté web et mobile)
  const playAudio = async (audioUrl: string) => {
    try {
      if (isWeb) {
        // Pour le web, utiliser l'API Audio standard
...
const startVideoCall = () => {
  ...
  return (
    ...
    >
      ...
      {/* Modal pour afficher l'historique complet */}
        ...
        <SafeAreaView style={styles.modalContainer}>
          {selectedContact && (
            <KeyboardAvoidingView
                 ...
                 >
              {/* En-tête simplifié avec seulement username, photo et bouton retour */}
              ...
              {/* Barre de saisie améliorée avec bouton d'attachement */}
              <View style={styles.simpleInputContainer}>
                {/* Bouton d'attachement (image/audio) */}
                <TouchableOpacity 
                ...
                {/* Bouton d'envoi ou indicateur d'enregistrement */}
                {isRecording ? (
