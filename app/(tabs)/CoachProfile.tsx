...
import * as FileSystem from "expo-file-system"; // For mobile base64 conversion
import {
  getUserIdFromToken,
  updateCoach,
  getCoachsById,
  ...
  getFullImageUrl,
  logout,
  getToken,  // Function to get the JWT token from storage
...
export default function CoachProfile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!coachId) {
          console.error("❌ Impossible de récupérer l'ID du coach.");
          setLoading(false);
          return;
        }
        const coachData = await getCoachsById(coachId); // Fetch coach data directly
        if (coachData) {
          setCoach(coachData);
          setName(coachData.username);
          setEmail(coachData.email);
        } else {
          console.error("❌ Aucun coach trouvé !");
        }
      } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await updateCoach(coach.id, updatedData);
  ...
  // Ouvrir la galerie et récupérer l'image (base64 = true)
  const pickImage = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      quality: 0.7,
      base64: true, // Sur web, c'est souvent un data URI
    });
  ...
  const handleUploadImage = async (uri: string) => {
    ...
    try {
      ...
      const filename = await uploadCoachPhoto(coach.id, fileUri);
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        <ActivityIndicator size="large" color="rgba(195, 0, 0, 0.2)" />
    ...
    return (
      <View style={styles.errorContainer}>
        <Text style={styles.errorText}>
          Erreur : Impossible de charger les données du coach.
        </Text>
      </View>
    );
  }
        ...
        <Svg height="100%" width="100%" style={styles.backgroundSvg}>
          <Defs>
            <LinearGradient id="grad" x1="0" y1="0" x2="1" y2="1">
              <Stop offset="0" stopColor="rgba(195, 0, 0, 0.2)" stopOpacity="0.7" />
              <Stop offset="1" stopColor="rgba(195, 0, 0, 0.2)" stopOpacity="0.5" />
            </LinearGradient>
          </Defs>
          <Circle cx="0" cy="0" r="150" fill="rgba(195, 0, 0, 0.2)" />
          <Circle cx="400" cy="100" r="80" fill="rgba(195, 0, 0, 0.2)" />
          <Circle cx="50" cy="450" r="120" fill="rgba(195, 0, 0, 0.2)" />
          <Circle cx="300" cy="600" r="200" fill="rgba(195, 0, 0, 0.2)" />
        </Svg>
      </View>
      
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.headerContainer}>
          <Text style={styles.title}>Mon Profil Coach</Text>
        ...
        <View style={styles.cardContainer}>
          <View style={styles.profileImageContainer}>
            <View style={styles.profileImageWrapper}>
              <Image
                source={
                  coach.photoUrl
                    ? { uri: getFullImageUrl(coach.photoUrl) }
          ...
          <View style={styles.formContainer}>
            ...
            {isEditing && (
              <View style={styles.inputGroup}>
                ...
                <View style={styles.inputWrapper}>
                  <Ionicons name="lock-closed" size={18} color="rgba(195, 0, 0, 0.6)" style={styles.inputIcon} />
                  <TextInput
                    style={styles.input}
                    value={password}
          ...
          <View style={styles.footerButtons}>
            <TouchableOpacity style={styles.backButton} onPress={() => router.push("/coach")}>
